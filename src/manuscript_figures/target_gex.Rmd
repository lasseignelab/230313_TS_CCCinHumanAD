...
```{r}
geo_target_info <- geo_output[["prioritization_tables"]][["ligand_activities_target_de_tbl"]] %>%
  filter(contrast == "AD-CTRL", receiver %in% receiver_oi) %>%
  ungroup() %>%
  select(target, logFC, receiver) %>%
  unique()  %>%
  mutate("dataset" = "geo")

gse_target_info <- gse_output[["prioritization_tables"]][["ligand_activities_target_de_tbl"]] %>%
  filter(contrast == "AD-CTRL", receiver %in% receiver_oi) %>%
  ungroup() %>%
  select(target, logFC, receiver) %>%
  unique()  %>%
...
target_info <- rbind(geo_target_info, gse_target_info)

target_info$receiver <- ifelse(
  target_info$receiver == "Inhibitory.Neurons",
  "Inhibitory Neurons",
  "Excitatory Neurons")
```
...
```
# grab targets of overlapping targets
```{r}
geo_targets <- lr_target_prior_cor_filtered_geo %>%
  filter(id %in% i4) %>%
  select(receiver, target, sender) %>%
  unique() %>%
  mutate("dataset" = "geo")

gse_targets <- lr_target_prior_cor_filtered_gse %>%
  filter(id %in% i4) %>%
  select(receiver, target, sender) %>%
  unique() %>%
  mutate("dataset" = "gse")

targets <- rbind(geo_targets, gse_targets) %>%
  unique()

targets_astro <- targets %>%
  filter(sender == "Astrocytes") %>%
  select(-sender) %>%
  unique() %>%
  left_join(target_info, by = c("receiver", "target", "dataset"))

targets_micro <- targets %>%
  filter(sender == "Microglia") %>%
  select(-sender) %>%
  unique() %>%
  left_join(target_info, by = c("receiver", "target", "dataset"))

targets_oligo <- targets %>%
  filter(sender == "Oligodendrocytes") %>%
  select(-sender) %>%
  unique() %>%
  left_join(target_info, by = c("receiver", "target", "dataset"))

targets_opcs <- targets %>%
  filter(sender == "OPCs") %>%
  select(-sender) %>%
  unique() %>%
  left_join(target_info, by = c("receiver", "target", "dataset"))
...
# astro heatmap
```{r}
astro_matrix <- dcast(targets_astro, target ~ receiver + dataset, value.var = "logFC") %>%
...
Heatmap(mat,
        ...
        row_title = "Astrocyte Target Genes",
...
# opc heatmap
```{r}
opc_matrix <- dcast(targets_opcs, target ~ receiver + dataset, value.var = "logFC") %>%
  column_to_rownames("target")
...
# micro heatmap
```{r}
micro_matrix <- dcast(targets_micro, target ~ receiver + dataset, value.var = "logFC") %>%
  column_to_rownames("target")
...
Heatmap(mat,
        ...
        show_column_dend = FALSE,
        cluster_columns = FALSE,
        row_title = "Microglia Target Genes",
        row_title_gp = gpar(fontface = "bold"),
        row_names_gp = gpar(fontface = "bold"),
        heatmap_legend_param = list(direction = "horizontal", title = "Pseudobulk Expression",
                                    labels_gp = gpar(fontface = "bold")))
...
# oligo heatmap
```{r}
oligo_matrix <- dcast(targets_oligo, target ~ receiver + dataset, value.var = "logFC") %>%
  column_to_rownames("target")
...
Heatmap(mat,
        ...
        show_column_dend = FALSE,
        cluster_columns = FALSE,
        row_title = "Oligodendrocyte Target Genes",
        row_title_gp = gpar(fontface = "bold"),
        row_names_gp = gpar(fontface = "bold"),
        heatmap_legend_param = list(direction = "horizontal", title = "Pseudobulk Expression",
                                    labels_gp = gpar(fontface = "bold")))
